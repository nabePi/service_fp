    ...
    function zkgetattendance($self) {
        ...
        if ( count($self->attendancedata) > 0 ) {
            ...
            while ( strlen($attendancedata) > 40 ) {
                ...
                // $u = unpack('H88', substr( $attendancedata, 0, 44) );
                //24s1s4s11s
                // print_r($u);
                // print_r("========\n");
                // print_r($u[1]);
                ...
                $uid = intval( $uid[0] );
                // $fooid = hexdec(substr( $u[1], 8, 18 ));
                // print_r("=======##\n");
                $_userid = substr($u[1],8,18);
            		$create_userid = '';
                ...
                // print_r($create_userid);
                $id = intval( str_replace("\0", '', hex2bin( substr($u[1], 6, 8) ) ) );
                // $state = hexdec(substr( $u[1], 56, 2));
                $state = hexdec(substr( $u[1], 67, 2));
				$timestamp = decode_time( hexdec( reverseHex( substr($u[1], 58, 8) ) ) );
				// 0000050035000000000000000000000000000000000000000000000000ed31581f0100000000ff
				/*print_r("###");
				print_r($u[1]);
				print_r("----");
				print_r(substr( $u[1], 56, 2));
				print_r("||||||");
				print_r(hexdec(00));
				print_r("<<<<<<<");
				print_r($state);*/
                ...
                array_push( $attendance, array( $create_userid, $timestamp));
